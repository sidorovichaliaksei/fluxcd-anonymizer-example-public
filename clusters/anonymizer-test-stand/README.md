...
> **`outboundRequestHostOverrides`** — раскомментируйте эту секцию в `services` (пример лежит в `anonymizer/helm-release.yaml`) **только если в вашей инфраструктуре домен анонимайзера переопределён через DNS** (split-horizon / DNS-оверрайд своего домена: тот же публичный адрес внутри кластера резолвится не туда, куда снаружи). Тогда обращения анонимайзера на свой публичный адрес ломаются, и секция перенаправляет их на внутренний сервис нужной роли. Правило: используете DNS-оверрайды своего домена — пропишите секцию; не используете — оставьте её закомментированной. В `host` укажите те же публичные адреса, что отдадите менеджеру на шаге 5.
