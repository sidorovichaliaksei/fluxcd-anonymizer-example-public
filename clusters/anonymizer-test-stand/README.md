...
### 6. (Опционально) `outboundRequestHostOverrides` — если у вас есть DNS-оверрайды своего домена

Anonymizer в части сценариев обращается на свой же публичный адрес (те, что вы настроили на шаге 5). Пока этот адрес во всей вашей сети резолвится одинаково — снаружи, через ваш ингресс с терминацией TLS — всё работает и эта секция не нужна.

Секцию нужно прописать, **если внутри вашей инфраструктуры домен анонимайзера переопределён через DNS (split-horizon / DNS-оверрайд своего домена)** — то есть тот же публичный адрес внутри кластера резолвится не туда, куда снаружи. В этом случае обращения анонимайзера на свой публичный адрес уходят не по назначению и ломаются. `outboundRequestHostOverrides` заставляет такие обращения идти напрямую на внутренний сервис нужной роли, минуя переопределённый DNS.

Правило простое: **используете DNS-оверрайды своего домена — пропишите секцию; не используете — оставьте её незаданной.**

В каждом элементе укажите публичный `host` из шага 5 и `role` (`regular` или `important`), чей внутренний сервис должен принимать эти обращения. Внутренний адрес чарт подставит сам:
